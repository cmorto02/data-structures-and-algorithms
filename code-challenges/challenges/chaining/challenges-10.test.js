...
const count = (target, input) => {
  const reduced = (acc, val) => val === target ? acc+1 : acc;
  const newArray = input.map(x=> x.reduce(reduced, 0));
...
const totalSum = (input) => {
  const reduced =(acc, val)=> acc+val;
  const mappedArray = input.map(x=> x.reduce(reduced));
  return mappedArray.reduce(reduced);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  const mapped = input.map(arr => arr.filter(x => x%5 === 0 && typeof x === 'number'));
  return mapped.map(arr => arr.map(x => Math.pow(2,x)))
};
...
let findMaleAndFemale = (data) => {
  ...
  return data.map(arr => arr.gender === 'male' || arr.gender === 'female' ? arr.name : null).filter(a => a !== null).join(' and ');
