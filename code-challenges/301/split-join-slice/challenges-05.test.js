...
const listFoods = (recipe) => {
  let result = [];
  recipe.ingredients.map(function(s) {
    result.push(s.split(/\s+/).slice(2,5).join(' '));
  })
  return result;
}
...
const stepActions = (recipe) => {
  ...
  recipe.steps.map(function(s) {
    result.push(s.split(/\s+/).slice(0,1).join(', '));
  })
  return result;
}
...
const extractVowels = (str) => {
  let arr = [str];
  arr.push(str.match(/[aeiou]/g).join(''));
  arr[0].replace(/[aeiou]/g, '');
  return arr;
};
